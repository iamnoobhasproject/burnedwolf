...
const TOOLS = [
    ...
    },
    {
        ...
        async run() {
            ...
            const { UPDATE_MANIFEST_URL, isNewerVersion } = require('../distribution');
            const join = UPDATE_MANIFEST_URL + (UPDATE_MANIFEST_URL.includes('?') ? '&' : '?') + 't=' + Date.now();
            const data = await fetchJSON(join);
            if (!data || !data.version) return { ok: false, error: 'Update server unreachable (check DNS/connection).', current: CURRENT_VERSION };
            return { ok: true, current: CURRENT_VERSION, latest: data.version, updateAvailable: isNewerVersion(data.version, CURRENT_VERSION) };
        }
    },
    {
        name: 'navigate_ui',
