...
const { isNewerVersion } = require('./distribution');
...
function checkForUpdate() {
    return new Promise((resolve) => {
        let manifestUrl;
        try {
            manifestUrl = new URL(UPDATE_MANIFEST_URL);
            manifestUrl.searchParams.set('t', String(Date.now()));
        } catch (e) {
            resolve({ state: 'error', error: 'manifest-url' });
            return;
        }
        ...
        const req = https.get(manifestUrl, {
            ...
            headers: {
                ...
                'Pragma': 'no-cache',
                'Expires': '0',
                'User-Agent': 'BurnedWolf/4.0 (+https://burnedwolf.whyscripts.com)'
            }
        }, (res) => {
            if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                resolve({ state: 'error', error: 'redirect' });
                res.resume();
                return;
            }
            if (res.statusCode !== 200) {
                resolve({ state: 'error', error: `http-${res.statusCode || 0}` });
                res.resume();
                return;
            }
            let data = '';
            ...
            res.on('end', () => {
                try {
                    ...
                    const version = String(serverData.version || '').trim();
                    const zipUrl = String(serverData.zipUrl || '').trim();
                    if (version && zipUrl && isNewerVersion(version, CURRENT_VERSION)) {
                        UPDATE_URL = zipUrl;
                        pendingUpdate = { current: CURRENT_VERSION, new: version, zipUrl };
                        resolve({ state: 'update', current: CURRENT_VERSION, new: version, zipUrl });
                    } else {
                        pendingUpdate = null;
