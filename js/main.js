/* BurnedWolf Website - Core runtime */
...
(function () {
  ...
  function detectInitialLang() {
    // Only honor a previously saved choice. On a brand-new visit we
    // always show the picker — the user explicitly asked for it.
    const saved = localStorage.getItem(STORAGE_KEY);
    if (saved && SUPPORTED.includes(saved)) return saved;
    return null; // means "ask user"
  }

  function previewLangForPicker() {
    // Pre-fill picker copy in a sensible language so it's readable
    // before the user picks. Use browser language hint, default to EN.
  ...
  /* ---------------------------------------------------------
     LANGUAGE OVERLAY (first-visit picker)
     --------------------------------------------------------- */
  ...
  function setupLangOverlay() {
    ...
    // Show overlay — render its copy in the most likely language
    ...
    tiles.forEach(tile => {
      tile.addEventListener('click', () => {
        ...
        // Live-preview the picked language on the overlay copy
        applyLang(picked);
      });
    });
